...
const ExperienceCard = ({ experience, onClick }: ExperienceProps) => {
  const { _id, title, description, price, imageUrl } = experience; 
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden transition-transform duration-300 hover:shadow-xl hover:-translate-y-1 flex flex-col w-full max-w-sm mx-auto">
      <div className="relative h-48 sm:h-56">
        <Image
          ...
          onError={() => {
            // Next/Image handles error states differently
            const imgElement = document.querySelector(`[alt="${title}"]`) as HTMLImageElement;
            if (imgElement) {
              imgElement.src = "https://placehold.co/400x250/E5E7EB/4B5563?text=Image+Unavailable";
            }
          }}
        />
      </div>
      ...
      <div className="p-4 sm:p-6 flex flex-col justify-between grow">
        <div>
          ...
          <p className="text-gray-600 text-xs mb-4 line-clamp-1">
            {description}
          </p>
        </div>
...
/**
 * Home content component with client-side functionality.
 */
